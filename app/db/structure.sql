...
CREATE TABLE public.outbox_events (
    ...
    created_at timestamp(6) without time zone NOT NULL,
    updated_at timestamp(6) without time zone NOT NULL,
    CONSTRAINT outbox_events_idempotency_payload_hash_format_check CHECK (((idempotency_key IS NULL) OR (created_at < '2026-02-21 21:00:00-03'::timestamp with time zone) OR ((payload ->> 'payload_hash'::text) ~ '^[0-9a-f]{64}$'::text))),
