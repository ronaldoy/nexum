class AddPendingReviewStatusToAnticipationRequests < ActiveRecord::Migration[8.2]
  def up
    remove_check_constraint :anticipation_requests, name: "anticipation_requests_status_check"
    add_check_constraint(
      :anticipation_requests,
      "status IN ('REQUESTED','PENDING_REVIEW','APPROVED','FUNDED','SETTLED','CANCELLED','REJECTED')",
      name: "anticipation_requests_status_check"
    )
  ...
  def down
    ...
    add_check_constraint(
      :anticipation_requests,
      "status IN ('REQUESTED','APPROVED','FUNDED','SETTLED','CANCELLED','REJECTED')",
      name: "anticipation_requests_status_check"
    )
