class AnticipationRequest < ApplicationRecord
  STATUSES = %w[REQUESTED PENDING_REVIEW APPROVED FUNDED SETTLED CANCELLED REJECTED].freeze
  CHANNELS = %w[API PORTAL WEBHOOK INTERNAL].freeze

  VALID_TRANSITIONS = {
    "REQUESTED" => %w[PENDING_REVIEW APPROVED CANCELLED REJECTED],
    "PENDING_REVIEW" => %w[REQUESTED CANCELLED REJECTED],
