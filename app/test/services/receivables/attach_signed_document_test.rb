...
module Receivables
  class AttachSignedDocumentTest < ActiveSupport::TestCase
    ...
    test "requires blob actor metadata when direct upload markers are present" do
      ...
      error = assert_raises(Receivables::AttachSignedDocument::ValidationError) do
        @service.send(
    ...
    test "does not enforce actor metadata when blob is not from direct upload flow" do
      blob = BlobDouble.new(
        metadata: {
          "tenant_id" => @tenant.id
        }
    ...
    def build_service
      Receivables::AttachSignedDocument.new(
        ...
        endpoint_path: "/api/v1/receivables/test",
        http_method: "POST"
      )
    end
  end
